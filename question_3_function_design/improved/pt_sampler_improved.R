#' Coursework target log density
#'
#' Evaluate the unnormalised log density used in the original MATH70093
#' coursework. This target has support on `[2, Inf)` and three modes. It is
#' kept separate from the sampler so that the sampler can accept other target
#' log-density functions.
#'
#' @param x Numeric vector of states.
#' @return A numeric vector of unnormalised log-density values.
#' @export
coursework_log_target <- function(x) {
...
#' target distribution or the sequence of random-number calls.
#'
...
#'   log_target = coursework_log_target,
#'   lower_bound = 2,
#'   n = 1000,
#'   x0 = rep(5, 3),
#'   sigmas = c(1, 3, 6),
...
# Representative settings copied from the original coursework run.
coursework_seed <- 1L
